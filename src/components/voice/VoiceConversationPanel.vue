...
async function leaveAfterGuidance() {
  ...
  voiceStore.reset()
  emit('close')
  if (isTransfer.value) await router.push({ name: 'transfer-home' })
  ending.value = false
}
...
<template>
  ...
  >
    ...
    >
      ...
      <p class="voice-card-guide">고르신 것이 맞으면 아래에서 한 번 더 확인해 주세요.</p>
    ...
    <div
      ...
      class="voice-slot-box"
    >
      <strong>아직 확인이 필요해요</strong>
      <div
        v-for="row in requiredSlotRows"
        :key="`required-${row.key}`"
        class="voice-slot-row"
      >
        <span>{{ row.label }}</span>
        <b>{{ row.value }}</b>
      </div>
    ...
    <div
      ...
      aria-label="보내려는 내용"
      class="voice-slot-box voice-draft-box"
    >
      <strong>보내려는 내용</strong>
      <div
        v-for="row in draftRows"
        :key="`draft-${row.key}`"
        class="voice-slot-row voice-draft-row"
      >
        <span>{{ row.label }}</span>
        <b>{{ row.value }}</b>
      </div>
    ...
    <div class="voice-actions">
      <Button
        v-if="voiceCaptureReady"
        class="voice-listen-button w-full"
        :disabled="busy"
        @click="listen"
      >
