...
export const useTransferStore = defineStore('transfer', () => {
  ...
  /** 출금 계좌 후보. 서버가 활성 계좌만 내려주도록 요청한다. */
  async function loadAccounts(params = { active: true }) {
    accountsLoading.value = true
    try {
      const response = await run(() => transfersApi.listAccounts(params))
      accounts.value = listOf(response, 'accounts')
  ...
  async function findRecipients(request) {
    ...
    candidates.value = listOf(response, 'candidates')
